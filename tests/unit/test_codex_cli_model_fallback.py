...
def test_run_codex_task_falls_back_from_gpt_5_5_on_invalid_model_error(monkeypatch) -> None:
    calls: list[list[str]] = []
    ...
    result = run_codex_task(
        task,
        "annotate file",
        CodexConfig(cli_path="codex", default_model="gpt-5.5"),
        LimitsConfig(),
        working_dir="/tmp/workdir",
    )
    ...
    assert calls[1][calls[1].index("--model") + 1] == "gpt-5.4"
